...
> 更新：2026-08-16 23:30 ｜ 状态口径：🟢已验证 / 🟡已实现待验证 / 🔴未实现 / ⚪平台或设计限制

## 1. 能力验收矩阵
| 能力 | 代码 | 部署 | 真机 | 备注 |
|---|---|---|---|---|
| 服务器桥 dodo_bridge_server.py | ✅ | ✅ | ✅ | 101.43.38.124:8080，ws_connected |
| 传输层：收→队列→pull→发→**原生引用** | ✅ | ✅ | ✅ | 08-16 群内初尘亲测引用回复成功 |
| at_only 群模式 | ✅ | ✅ | — | 服务器已配 |
| 丢 N 分钟前内容（stale） | ✅ | ✅ | — | STALE_MSG_TTL_SECONDS=300 |
| 批量领取（BUG-02） | ✅ | ✅ | — | /api/pull?count=N |
| 队列管理（BUG-03） | ✅ | ✅ | — | /api/queue/clear+stats |
| 会话限流+队列上限（BUG-04） | ✅ | ✅ | — | |
| P2 Operit 消费端（com.operit.napcat_pro） | ✅ | ✅烧录 | 🟡 | IPC 修复版；工具需新会话调用；**受控测试未完** |
| 对话绑定 fixed/auto | ✅ | — | 🟡 | fixed 已绑渡渡对话 |
| 选择性忽略哨兵 | ✅ | — | — | [[QQ_BRIDGE_IGNORE]] |
| **G1 群聚合桶**（P3） | ✅ | ✅ | — | 默认关（group_aggregate_window_ms=0） |
| **G2 上下文双模式** time/count（P3） | ✅ | ✅ | — | 默认 count（需求16/17） |
| **G3 replyTo 协议头**（P3） | ✅ | ✅ | — | `[replyTo:N]`/JSON→原生引用编号消息（聚合轮） |
| **G7 成员别名映射**（P3） | ✅ | ✅ | — | known_users，需求18：3297828886→苜蓿 已配 |
...
## 2. 当前运行状态（2026-08-16 23:30）
- 服务器桥：running（ws_connected=true），0.0.0.0:8080
- 自动回复：**关**（Operit config enabled=false）
- 群模式：at_only（只回艾特）｜ 上下文：count 模式 ｜ 聚合：关
- 绑定：fixedChatId=2f928270-e930-4752-834d-e14485a44a87「初尘&渡渡·第三次桥接」，角色卡渡渡（**Operit 重载后生效**）
- 队列：通常空（自动回复关着，只有 /health 等查询）

## 3. 需求清单（详见 DESIGN.md §2.2，13~17）
13 默认不自动开 ✅ ｜ 14 群默认 at_only ✅ ｜ 15 丢 N 分钟前内容 ✅ ｜ 16 at_only 读艾特前后 N 秒（time 模式 ✅）｜ 17 上下文双模式 time/count ✅（G2）
