...
- **进度**：P0 ✅ P1 ✅（服务器侧部署+传输层验证）P2 ✅（Operit 消费端代码+烧录，IPC 修复版）P3 服务器侧 G1 聚合桶 + G2 上下文双模式 ✅（28/28，已部署）；**P3 剩 G3 replyTo / G7 成员映射 / Operit 侧配合** → P4 附加层 → P5 UI+GitHub。
- **必须读**：`STATUS.md`（当前状态）→ `ARCHITECTURE.md`（架构）→ `DESIGN.md`（需求/决策）→ `TROUBLESHOOTING.md`（坑）→ `IDEAS.md`（待办/灵感/URL）。
- **铁律**：自动回复默认关；测试前**必须问初尘**：绑哪个对话 / 哪个群 / 开不开；新工具当前会话不可见需新开会话。
